    ...
    <div className="p-8 bg-white min-h-screen text-black">
      <header className="flex justify-between items-center mb-8 border-b pb-6">
        ...
        <button 
          ...
          className="bg-[#16223F] text-white px-6 py-3 rounded-xl font-bold hover:bg-[#16223F]/90 shadow-lg transition-transform active:scale-95"
        >
          + Add New Entry
        </button>
      </header>
      ...
      <div className="bg-white border rounded-2xl overflow-hidden shadow-sm">
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#16223F]/5 border-b border-gray-200 text-[#16223F] uppercase text-[10px] font-black tracking-widest">
            <tr>
              <th className="p-4 text-xs font-bold uppercase">Date/Time</th>
              {currentModule.fields.map(f => (
                <th key={f.name} className="p-4 text-xs font-bold uppercase">{f.label}</th>
              ))}
            </tr>
          </thead>
