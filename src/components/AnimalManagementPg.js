...
const AnimalManagementPg = () => {
  ...
  const fetchAnimals = async () => {
    setIsLoading(true);
    try {
      const data = await api.animals.getAll();
      setAnimals(data || []);
    } catch (err) {
      console.error(err);
      swalError("Error", "Failed to retrieve animals.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSave = async (e) => {
    ...
    if (!formData.name.trim()) {
      swalError("Error", "Animal Name is required");
      return;
    }
    if (!formData.code.trim()) {
      swalError("Error", "Animal Code is required");
      return;
    }
    ...
    try {
      ...
      if (formData.id) {
        await api.animals.update(formData.id, payload);
        swalSuccess("Success", "Animal updated successfully");
      } else {
        await api.animals.create(payload);
        swalSuccess("Success", "Animal registered successfully");
      }
      setShowForm(false);
      fetchAnimals();
    } catch (err) {
      console.error(err);
      swalError("Error", typeof err === "string" ? err : "Failed to save animal details");
    } finally {
  ...
  const handleEdit = (animal) => {
    setFormData({
      ...
      status: animal.status !== undefined ? animal.status : true,
    });
    setShowForm(true);
  };

  const handleDelete = async (id) => {
    const confirmed = await swalConfirm("Delete Animal Type?", "Are you sure you want to delete this animal configuration?");
    if (!confirmed) return;

    try {
      await api.animals.delete(id);
      swalSuccess("Deleted", "Animal deleted successfully");
      fetchAnimals();
    } catch (err) {
      console.error(err);
      swalError("Error", "Failed to delete animal");
    }
  };
      ...
      <div className="flex-none flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 font-medium mt-1">
            Configure dynamic animal types to populate options in livestock management forms.
          </p>
        </div>
        ...
        {(isLoading || filteredAnimals.length > 0) && (
          <table className="w-full text-left min-w-[600px] relative">
            <thead className="sticky top-0 z-10 bg-gray-50 text-[#16223F] uppercase text-[10px] font-black tracking-widest shadow-sm">
              <tr>
                <th className="p-4 border-b">Animal Name</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                filteredAnimals.map((animal) => (
                  <tr key={animal.id || animal._id} className="hover:bg-[#D1867D]/5 transition-colors">
                    ...
                    <td className="p-4 text-center">
                      ...
                      >
                        {animal.status === true ? "ACTIVE" : "INACTIVE"}
                      </span>
                    </td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-[30px] p-8 w-full max-w-lg shadow-2xl relative">
            ...
            <h2 className="text-2xl font-black text-[#071437] mb-6 pr-10">
              {formData.id ? "Edit Animal Type" : "Register Animal Type"}
            </h2>
            ...
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleChange}
                  placeholder="Details about characteristics..."
              ...
              <div className="flex gap-4 mt-8 pt-4 border-t border-[#edf1f7]">
                ...
                >
                  {isLoadingForm ? "Saving..." : "Save Animal"}
                </button>
                <button
                  type="button"
                  onClick={() => setShowForm(false)}
...
export default AnimalManagementPg;
