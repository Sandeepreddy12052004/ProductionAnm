...
const { data: users, error, mutate, isLoading } = useSWR('users_cache', fetcher, {
  ...
  const filteredUsers = safeUsers.filter(user => {
    ...
    // 3. Check inline Department filter
    if (selectedDepartment) {
      const userDeptName = user.department && typeof user.department === 'object'
        ? user.department.name
        : user.department;
      if (String(userDeptName).toLowerCase() !== selectedDepartment.toLowerCase()) return false;
    }
    ...
    if (selectedStatus) {
      ...
      if (userStatusStr.toLowerCase() !== selectedStatus.toLowerCase()) return false;
    }
  ...
  return (
    <div className="p-4 md:p-8 w-full h-full flex flex-col bg-transparent text-slate-800">
      ...
      <div className="flex-none flex flex-wrap items-end gap-3.5 mb-6 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm w-full">
        ...
        <div className="flex-1 min-w-[280px]">
          <div className="relative">
            ...
            <input
              ...
              className="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-200 text-sm bg-slate-50/30 text-[#16223F] placeholder-slate-400 focus:bg-white focus:border-[#D1867D] focus:ring-2 focus:ring-[#D1867D]/10 outline-none transition-all duration-200 font-medium"
            />
          </div>
        </div>
        ...
        {/* Department Filter */}
        <div className="flex flex-col gap-1 min-w-[130px]">
          <label className="text-[11px] font-bold text-slate-500 tracking-wide ml-0.5">Department</label>
          <div className="relative">
            <select
        ...
        {/* Role Filter */}
        <div className="flex flex-col gap-1 min-w-[130px]">
          <label className="text-[11px] font-bold text-slate-500 tracking-wide ml-0.5">Role</label>
          <div className="relative">
            <select
              value={selectedRole}
        ...
        {/* Status Filter */}
        <div className="flex flex-col gap-1 min-w-[130px]">
          <label className="text-[11px] font-bold text-slate-500 tracking-wide ml-0.5">Status</label>
          <div className="relative">
            <select
