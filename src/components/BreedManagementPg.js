...
const BreedManagementPg = () => {
  ...
  const filterFields = [
    { name: "name", label: "Breed Name", type: "text" },
      ...
      for (const f of fieldFilters) {
        ...
        if (f.field === "status") {
          ...
          if (selectedValues.length > 0) {
            const breedStatus = breed.status !== false ? "active" : "inactive";
            const optionMatched = selectedValues.some(v => String(v).toLowerCase() === breedStatus);
            if (!optionMatched) currentMatch = false;
          }
        } else {
          if (f.value) {
            currentMatch = String(breed[f.field] || "")
              .toLowerCase()
